...
stdenv.mkDerivation (final: {
  ...
  deps = callPackage ./deps.nix { name = "${final.pname}-cache-${final.version}"; };

  zigBuildFlags = [
    "--system"
    "${final.deps}"
    "-Dversion-string=${final.version}"
  ];

  zigCheckFlags = final.zigBuildFlags;
  ...
  meta = {
    ...
    mainProgram = "ghostty";
    outputsToInstall = final.outputs;
